# executing the entire procedure 10 times in a loop 
...
ensembles = data.frame(majority = numeric(), boost = numeric(), full = numeric(), bayesian = numeric())
...
accuracy = c(C5.0.conf658$overall[1], gbm.conf127$overall[1], glmn.conf127$overall[1], linsvm.conf127$overall[1],
                       nb.conf5$overall[1], nnet.conf658$overall[1], nnet.conf127.overs$overall[1], nnet.conf127$overall[1],
                       pda.conf127$overall[1], pda.conf127.overs$overall[1], rf.conf658$overall[1], rf.conf127$overall[1])

sensitivity = c(C5.0.conf658$byClass[1], gbm.conf127$byClass[1], glmn.conf127$byClass[1], linsvm.conf127$byClass[1],
                        nb.conf5$byClass[1], nnet.conf658$byClass[1], nnet.conf127.overs$byClass[1], nnet.conf127$byClass[1],
                        pda.conf127$byClass[1], pda.conf127.overs$byClass[1], rf.conf658$byClass[1], rf.conf127$byClass[1])

specificity = c(C5.0.conf658$byClass[2], gbm.conf127$byClass[2], glmn.conf127$byClass[2], linsvm.conf127$byClass[2],
                           nb.conf5$byClass[2], nnet.conf658$byClass[2], nnet.conf127.overs$byClass[2], nnet.conf127$byClass[2],
                           pda.conf127$byClass[2], pda.conf127.overs$byClass[2], rf.conf658$byClass[2], rf.conf127$byClass[2])




accuracies = rbind(accuracies,accuracy)
specificities = rbind(specificities, specificity)
sensitivities = rbind(sensitivities, sensitivity)
...
maj_res = exhaustive_ens_search(predlist, sentitest)
...
# exhaustively searches the ensemble space for all possible model combinations
# optionally a vector of weights for every model can be specified
boost_res = exhaustive_ens_search(preds_test, sentitest, totalalpha)

#add boosted models to the full model list
predlist = cbind(predlist,  preds_test[,c(2,4,6)]) 

#then do exhaustive search (with 14 models we have 2^14 = 16384 possible combinations)
full_res = exhaustive_ens_search(predlist, sentitest)
...
ensemblepreds = c(maj_res$accuracy, boost_res$accuracy, full_res$accuracy, bayes_acc)
ensembles = rbind(ensembles, ensemblepreds)
